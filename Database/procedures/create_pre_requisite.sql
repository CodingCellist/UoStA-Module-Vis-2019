-- creates a new pre-requisite, containing at least one module
CREATE PROCEDURE teh6_2019_summer_project.create_pre_requisite(
...
BEGIN
	...
	*/
	...
	IF (source_module_code IN (SELECT source_module FROM teh6_2019_summer_project.requisite WHERE
								...
								AND requisite.`type` = 'Pre'
								)
							 )
	 	)
 	THEN
 		SIGNAL SQLSTATE '45001' SET message_text = "A pre-requisite for that module, in that year and semester, and at that level already exists. Perhaps you meant to modify one of those, or add an alternative pre-requisite?";
 	END IF;
 	*/
	...
	VALUES
	(academic_year_concerned, semester_concerned, academic_level_concerned, 'Pre', source_module_code);
